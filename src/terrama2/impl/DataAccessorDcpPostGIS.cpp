...
std::string terrama2::core::DataAccessorDcpPostGIS::getTableName(const DataSetDcp& datasetDcp) const
{
  try
  {
    return datasetDcp.format.at("table_name");
  }
  catch (...)
  {
    //TODO: log this
...
std::shared_ptr<te::mem::DataSet> terrama2::core::DataAccessorDcpPostGIS::getDataSet(const std::string& uri, const Filter& filter, const DataSetDcp& datasetDcp) const
{
  QUrl url(uri.c_str());

  std::string tableName = getTableName(datasetDcp);
