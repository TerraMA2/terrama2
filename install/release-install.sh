...
sudo apt-get update
sudo apt-get install -y cmake git libquazip-dev unzip screen openssh-server doxygen supervisor locales libgsasl7 postgresql-11-postgis-2.5 postgis postgresql-server-dev-11 nodejs python-psycopg2
mkdir $HOME/.ssh
