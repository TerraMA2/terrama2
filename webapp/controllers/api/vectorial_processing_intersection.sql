...
$$
  SELECT ARRAY(
    SELECT 
        terrama.id 
    FROM 
        dblink('host=localhost port=5432 user=postgres password=postgres dbname=terrama2', 
            'SELECT 
                    id::text
            FROM 
                    terrama2.analysis
            WHERE automatic_schedule_id is not null') 
                AS terrama(
                    id text))
$$
...
IF array_length(list_id, 1) > 0 THEN
    ...
    LOOP
        ...
        EXECUTE format((
            SELECT terrama.count_value
            FROM dblink(
                'host=localhost port=5432 user=postgres password=postgres dbname=terrama2', 
                'SELECT COUNT(*) as count_value FROM terrama2.analysis_metadata WHERE key = ''dynamicDataSeries'' AND analysis_id = ' || CAST(list_id[indice] AS INTEGER))
                AS terrama(
                    count_value INTEGER
                )
        )) INTO number_of_rows;
        ...
        IF number_of_rows > 0 THEN
            EXECUTE format((
				SELECT terrama.value
				FROM dblink(
					'host=localhost port=5432 user=postgres password=postgres dbname=terrama2', 
					'SELECT value FROM terrama2.analysis_metadata WHERE key = ''dynamicDataSeries'' AND analysis_id = ' || CAST(list_id[indice] AS INTEGER))
					AS terrama(
						value text
					)
			)) INTO id_analysis;
            ...
            EXECUTE format((
				...
				FROM dblink(
					'host=localhost port=5432 user=postgres password=postgres dbname=terrama2', 
					'SELECT automatic_schedule_id FROM terrama2.analysis WHERE automatic_schedule_id is not null AND id = ' || CAST(list_id[indice] AS INTEGER))
					AS terrama(
						automatic_schedule_id integer
						)
					)) INTO automatic_schedule_id_selected;
...
BEGIN
    ...
    EXECUTE format(
        E'SELECT terrama.count_value
          FROM dblink(
              \'host=localhost port=5432 user=postgres password=postgres dbname=terrama2\',
              \'SELECT COUNT(*) count_value FROM terrama2.analysis_metadata WHERE key = \'\'last_analysis\'\' AND terrama2.analysis_metadata.analysis_id = %s\')
          AS terrama(
              count_value INTEGER
          )', analysis_id
    ) INTO number_of_rows_metadata;

    EXECUTE format(
        E' SELECT terrama.value
           FROM dblink(
            \'host=localhost port=5432 user=postgres password=postgres dbname=terrama2\',
            \'SELECT MAX(value::TIMESTAMP) AS value FROM terrama2.analysis_metadata WHERE key = \'\'last_analysis\'\' AND terrama2.analysis_metadata.analysis_id = %s\')
            AS terrama(
                value INTEGER
            )', analysis_id
        ) INTO last_analysis;
    ...
    EXECUTE format(
        E' SELECT dblink_exec(
            \'host=localhost port=5432 user=postgres password=postgres dbname=terrama2\',
            \'INSERT INTO terrama2.analysis_metadata(key, value, analysis_id) VALUES (\'\'last_analysis\'\', \'\'%s\'\', %s)\');', 
